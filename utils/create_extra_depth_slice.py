...
-----------------------------------------------------------------------------------------------
 ...
 DESCRIPTION
   ...
   Eta, created at the corresponding depth and with the same resolution. To compute the bulk
   sound speed (also needs Vpv, Vph, Vsv, Vsh, and Eta), use parameter 'cb' (or 'dcb', in case
   you want the perturbations). The same applies to the Vp/Vs (use parameter 'vpvs'). To
   calculate the transverse isotropy from Vsv and Vsh, use the parameter 'ti'. The perturbations
...
if __name__ == '__main__':
  ...
  if prm == 'iso' or prm == 'cb' or prm == 'vpvs' or prm == 'diso' or prm == 'dcb':
    ...
    file = '{}_{}_DS.dat'.format (prm[1:], depth)

    depth, nlat, nlon, theta, phi, M = readGridFile (file)
    ...
    name  = '{}_{:g}_DS.dat'.format (prm, depth)
