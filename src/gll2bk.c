...
-----------------------------------------------------------------------------------------------
 ...
 USAGE
   mpiexec -n 12 bin/gll2bk MIN_DEPTH MAX_DEPTH HORIZONTAL_RESOLUTION RADIAL_RESOLUTION
                            INPUT_DIRECTORY OUTPUT_DIRECTORY

 EXAMPLE
   mpiexec -n 12 bin/gll2bk -5 80 0.5 1.0 data/INPUT/ .
 ...
 DESCRIPTION
   ...
   input and output directory names from the command line and creates a block model for the
   desired parameters (defined in 'config.h'). The routine writes the output to binary files
   called PARAMETER.bin.
...
static unsigned optimize (int ic, unsigned nel,
                          ...
                          struct Point To[np_l][nt][nr],
                          struct CartesianPoint spf[nel][NN],
                          struct Parameters Mo_l[np_l][nt][nr],
                          struct Parameters a[nel][NX * NY * NZ])
{
  ...
  for (unsigned k = 0; k < nr; k++)
  {
    for (unsigned j = 0; j < nt; j++)
      ...
      for (unsigned i = 0; i < np_l; i++)
      {
        ...
        if (newton (&xi, &eta, &gamma, nel, eijk.element, &To[i][j][k], spf))
        {
        ...
        surface (&Mo_l[i][j][k], nel, eijk.element, xi, eta, gamma, a);
      }
...
int main (int argc, char *argv[])
{
  ...
  createOutputGridBk (ic, nc, k, kk, r1, r2, np, np_l, nt, nr, &gb, To);
  ...
  if (checkTopoAndModelIO (scanMeshAndModel (ic, argv[5], NEL, &gb, lle, llm,
  ...
  if (findFirstGuesses (ic, nel, np_l, nt, nr, b, f, Ti, To, Eijk))
  {
  ...
  if (optimize (ic, nel, np_l, nt, nr, X, Y, Z, Eijk, To, spf, Mo_l, a))
  {
    fprintf (stderr, "Singular Jacobian. No solution found.\n");
  ...
  if (ic == 0)
  {
    ...
    if (checkModelIO (writeModelBk (argv[6],
                                    np, nt, nr, Mo))) MPI_Abort (MPI_COMM_WORLD, 1);
  }
